...
export const addCard = (card) => ({ type: actionType.ADD_DESK, payload: { card }});
export const removeCard = (removeId) => ({ type: actionType.REMOVE_DESK, payload: { removeId }});
export const setCards = (cards) => ({ type: actionType.SET_DESKS, payload: { cards }});
...
export const createDesk = (name) => (dispatch) => (
  api.createDesk(name)
    ...
    .catch(() => dispatch({ type: 'createDesksFail'}))
)
